    ...
    <a-form :form="form">
      <a-form-item label='帐号'
        ...
        <a-input v-model="user.username"
                 ...
                 v-decorator="['username',{rules: [{ required: true, message: '帐号不能为空'}]}]"/>
      </a-form-item>
      <a-form-item label='用户名'
                   v-bind="formItemLayout"
                   :validateStatus1="validateStatus"
                   :help="help1">
        <a-input v-model="user.realName"
                 @blur="handleRealNameBlur"
                 v-decorator="['realName',{rules: [{ required: true, message: '用户名不能为空'}]}]"/>
      </a-form-item>
...
export default {
  ...
  methods: {
    ...
    handleUserNameBlur () {
      ...
      if (username.length) {
        if (username.length > 10) {
          ...
          this.help = '帐号不能超过10个字符'
        } else if (username.length < 4) {
          this.validateStatus = 'error'
          this.help = '帐号不能少于4个字符'
        } else {
    ...
    handleRealNameBlur () {
      let realName = this.user.realName.trim()
      if (realName.length) {
        if (realName.length > 30) {
          this.validateStatus1 = 'error'
          this.help1 = '用户名不能超过30个字符'
        } else if (realName.length < 1) {
          this.validateStatus1 = 'error'
          this.help1 = '用户名不能少于1个字符'
        }
      } else {
        this.validateStatus1 = 'error'
        this.help1 = '用户名不能为空'
